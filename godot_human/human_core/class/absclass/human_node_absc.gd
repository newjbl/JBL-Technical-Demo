...
func set_human_data():
	...
	human_dir = "res://human_core/res/extract__base/"
	human_look_o.skeleton_path = "res://human_core/res/skeleton_temp.tscn"
	human_look_o.ani_tree_path = "res://human_core/ani/ani_temp.tscn"
	human_look_o.mesh_body = human_dir.path_join("mesh__CC_Base_Body_all.tres")
	human_look_o.mesh_eye = human_dir.path_join("mesh__CC_Base_Eye_all.tres")
	human_look_o.mesh_eyeo = human_dir.path_join("mesh__CC_Base_EyeOcclusion_all.tres")
	human_look_o.mesh_tear = human_dir.path_join("mesh__CC_Base_TearLine_all.tres")
	human_look_o.mesh_tee = human_dir.path_join("mesh__CC_Base_Teeth_all.tres")
	human_look_o.mesh_tong = human_dir.path_join("mesh__CC_Base_Tongue_all.tres")
...
func update_mesh_infor(a:String, c:String, d:String, e:String):
	if a != '':
		var amesh = load(a)
		...
		b.name = d
		#var blend_data = load_blend_shape_bin(e)
		#for shapename in blend_data:
		#	apply_blend_shape(b, amesh, shapename, blend_data[shapename])
		b.mesh = amesh
		return b
	return null
...
func create_human_mesh():
	...
	human_body_mesh3d = update_mesh_infor(human_look_o.mesh_body, human_look_o.skin_body, 'body', human_look_o.blend_body)
	human_eye_mesh3d = update_mesh_infor(human_look_o.mesh_eye, human_look_o.skin_eye, 'eye', human_look_o.blend_eye)
	human_eyeo_mesh3d = update_mesh_infor(human_look_o.mesh_eyeo, human_look_o.skin_eyeo, 'eyeo', human_look_o.blend_eyeo)
	human_tear_mesh3d = update_mesh_infor(human_look_o.mesh_tear, human_look_o.skin_tear, 'tear', human_look_o.blend_tear)
	human_tee_mesh3d = update_mesh_infor(human_look_o.mesh_tee, human_look_o.skin_tee, 'tee', human_look_o.blend_tee)
	human_tong_mesh3d = update_mesh_infor(human_look_o.mesh_tong, human_look_o.skin_tong, 'tong', human_look_o.blend_tong)
	if human_noded != null:
...
func apply_blend_shape(mesh3d:MeshInstance3D, mesh: ArrayMesh, shape_name: String, surface_data: Array) -> void:
	var shape_idx = mesh3d.find_blend_shape_by_name(shape_name)
	if shape_idx == -1:
		return
